  ...
  return (
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-[15px] font-semibold text-slate-900">Library Announcements</h3>
          <p className="text-[12px] text-slate-400 mt-0.5">Institutional updates</p>
        </div>
            ...
            <div key={item.id} className="flex items-start gap-3 p-2.5 rounded-lg hover:bg-slate-50 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${status_colors[item.status] ?? 'bg-slate-100 text-slate-500'}`}>
                    {item.status}
                  </span>
                </div>
