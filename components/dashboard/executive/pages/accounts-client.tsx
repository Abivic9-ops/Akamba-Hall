...
export function AccountsClient() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-[28px] font-medium text-slate-900">Account Lookup</h1>
        <p className="text-[15px] text-slate-500 mt-1">Search and manage user accounts</p>
      </div>
      <div className="relative">
        <UserSearch className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400" />
        <input
          type="text"
          value={query}
          ...
          {filtered.map((a, i) => (
            <div key={i} className="flex items-center gap-4 px-6 py-3 hover:bg-slate-50/50 transition-colors">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-slate-400">{a.id} · {a.email} · Last login: {a.lastLogin}</p>
              </div>
