...
export function AccountsClient({ users }: { users: User[] }) {
            ...
            <div key={i} className="flex items-center gap-4 px-6 py-3 hover:bg-slate-50/50 transition-colors">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="text-[13px] font-medium text-slate-800 dark:text-[#E2E8F0]">{a.fullName ?? 'Unknown'}</p>
                  <span className="text-[10px] text-slate-400 dark:text-[#6B7A99] dark:text-[#6B7A99]">{a.role}</span>
                </div>
                <p className="text-[11px] text-slate-400 dark:text-[#6B7A99] dark:text-[#6B7A99]">{a.studentId ?? a.id.slice(0, 8)} · {a.email ?? 'N/A'} · Last login: {a.lastActiveAt ? new Date(a.lastActiveAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : 'Never'}</p>
              </div>
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${status_colors[a.status] ?? 'bg-slate-100 text-slate-600'}`}>{a.status}</span>
