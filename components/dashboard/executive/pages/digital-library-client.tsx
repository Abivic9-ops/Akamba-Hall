...
export function DigitalLibraryClient() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-[28px] font-medium text-slate-900">Digital Library</h1>
        <p className="text-[15px] text-slate-500 mt-1">Access e-resources and online databases</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {databases.map((db) => (
          <div key={db.name} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-3 mb-3">
              <div className="h-10 w-10 rounded-xl bg-teal-50 flex items-center justify-center">
                <Globe className="h-5 w-5 text-[#0D9488]" />
              </div>
              <div>
                <p className="text-[14px] font-semibold text-slate-800">{db.name}</p>
                <p className="text-[11px] text-slate-400">{db.access}</p>
              </div>
            </div>
            <p className="text-[12px] text-slate-500 mb-3">{db.description}</p>
