  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB]">
      <div className="max-w-[1200px] mx-auto p-6 space-y-6">
        ...
        {/* header */}
        <div>
          <h1 className="text-[28px] font-medium text-slate-900">Catalogue</h1>
          ...
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full h-12 pl-11 pr-4 rounded-xl border-2 border-slate-200 bg-white text-[15px] text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/10 transition-all"
              />
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {filtered.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="bg-white rounded-xl border border-slate-100 overflow-hidden hover:shadow-md transition-all group"
                >
                  ...
                  <div className="p-4">
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <span className="text-[12px] text-slate-500">{item.rating}</span>
                    </div>
                    <p className="text-[15px] font-medium text-slate-800 truncate">{item.title}</p>
                    <p className="text-[13px] text-slate-500 mt-0.5">{item.author}</p>
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <span className="text-[12px] text-slate-400">{item.copies} copies</span>
                    </div>
                  </div>
                </div>
              )
          ...
          <div className="bg-white rounded-xl border border-slate-100 overflow-hidden">
            {filtered.map((item) => {
              ...
              return (
                ...
                >
                  <div className="w-10 h-14 rounded-lg bg-slate-100 flex items-center justify-center shrink-0">
                    <BookOpen className="h-4 w-4 text-slate-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[15px] font-medium text-slate-800 truncate">{item.title}</p>
                    <p className="text-[13px] text-slate-500">{item.author} · {item.year}</p>
                  </div>
                  <div className="hidden sm:flex items-center gap-2">
                    <Star className="h-3.5 w-3.5 text-amber-400 fill-amber-400" />
                    <span className="text-[13px] text-slate-500">{item.rating}</span>
                  </div>
