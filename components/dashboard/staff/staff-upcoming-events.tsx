...
export function StaffUpcomingEvents({ events }: StaffUpcomingEventsProps) {
  return (
    ...
    >
      {events.length === 0 ? (
        <p className="text-[14px] text-slate-400 text-center py-6">No upcoming events</p>
      ) : (
          ...
          {events.map((event) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[14px] font-medium text-slate-800">{event.title}</p>
                <p className="text-[12px] text-slate-400 mt-0.5">{event.time} · {event.venue}</p>
              </div>
