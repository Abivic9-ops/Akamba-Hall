  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] dark:bg-[#071224] dark:bg-[#071224]">
      <div className="max-w-[1200px] mx-auto p-6 space-y-5">
        <div>
          <h1 className="text-[28px] font-medium text-slate-900 dark:text-[#E2E8F0] dark:text-[#E2E8F0]">Help Centre</h1>
          <p className="text-[15px] text-slate-500 dark:text-[#6B7A99] dark:text-[#6B7A99] mt-1">Get help with library services, policies, and account management.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {quickLinks.map((link) => {
            ...
            return (
              <a key={link.label} href={link.href} className="bg-white dark:bg-[#0E1F3F] dark:bg-[#0E1F3F] rounded-xl border border-slate-100 dark:border-white/[0.08] dark:border-white/[0.08] shadow-sm dark:shadow-none dark:shadow-none p-4 flex items-center gap-3 hover:shadow-md hover:border-[#2563EB]/20 transition-all group">
                ...
                <span className="text-[13px] font-medium text-slate-700 dark:text-[#E2E8F0]">{link.label}</span>
              </a>
            )
          })}
        </div>
          ...
          <div className="space-y-0">
            {faqs.map((faq) => (
              <div key={faq.id} className="py-3.5 border-b border-slate-50 last:border-0">
                <p className="text-[14px] font-medium text-slate-800 dark:text-[#E2E8F0] dark:text-[#E2E8F0]">{faq.question}</p>
                <p className="text-[13px] text-slate-500 dark:text-[#6B7A99] dark:text-[#6B7A99] mt-1.5 leading-relaxed">{faq.answer}</p>
              </div>
            ))}
          </div>
        </SectionCard>
        ...
        <SectionCard title="Contact Support" icon={MessageCircle}>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {contacts.map((contact) => {
              ...
              return (
                <div key={contact.label} className="flex items-center gap-3 p-3 rounded-xl bg-[#F8F9FB] dark:bg-[#071224] dark:bg-[#071224] hover:bg-slate-100 dark:bg-white/[0.06] dark:bg-white/[0.06] dark:bg-white/[0.06] transition-colors">
                  <div className="h-9 w-9 rounded-lg bg-white dark:bg-[#0E1F3F] dark:bg-[#0E1F3F] flex items-center justify-center shrink-0 border border-slate-100 dark:border-white/[0.08] dark:border-white/[0.08]">
                    <Icon className={`h-4 w-4 ${contact.color}`} />
                  </div>
                  <div>
                    <p className="text-[12px] text-slate-400 dark:text-[#6B7A99] dark:text-[#6B7A99]">{contact.label}</p>
                    <p className="text-[13px] font-medium text-slate-700 dark:text-[#E2E8F0]">{contact.value}</p>
                  </div>
                </div>
              )
            })}
