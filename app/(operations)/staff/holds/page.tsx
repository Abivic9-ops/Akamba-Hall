  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB]">
      <div className="max-w-[1200px] mx-auto p-6 space-y-5">
        <div>
          <h1 className="text-[28px] font-medium text-slate-900">Holds & Reservations</h1>
          <p className="text-[15px] text-slate-500 mt-1">Manage book holds, pickups, and reservation queues.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <SectionCard title="Ready for Pickup">
            <p className="text-[32px] font-bold text-[#18A957]">{readyCount}</p>
            <p className="text-[13px] text-slate-400 mt-1">Awaiting collection at the desk</p>
          </SectionCard>
          <SectionCard title="In Queue">
            <p className="text-[32px] font-bold text-[#F97316]">{pendingCount}</p>
            <p className="text-[13px] text-slate-400 mt-1">Members waiting for availability</p>
          </SectionCard>
          <SectionCard title="Total Holds">
            <p className="text-[32px] font-bold text-slate-900">{holds.length}</p>
            <p className="text-[13px] text-slate-400 mt-1">Active holds across all members</p>
          </SectionCard>
        </div>
        ...
        <SectionCard title="Active Holds" icon={Bookmark} cta={{ label: 'Process Pickup', href: '#' }}>
          <div className="space-y-0">
            {holds.map((hold) => {
              ...
              return (
                <div key={hold.id} className="flex items-center gap-4 py-3 border-b border-slate-50 last:border-0 hover:bg-slate-50 rounded-lg px-2 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[12px] text-slate-400 mt-0.5">{hold.author}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <Badge variant={cfg.variant} className="text-[10px]">{cfg.label}</Badge>
                    {hold.status === 'ready' && hold.pickupDeadline && (
                      <p className="text-[11px] text-slate-400 mt-1">Pick up by {hold.pickupDeadline}</p>
                    )}
                    {hold.status === 'pending' && hold.queuePosition !== null && (
                      <p className="text-[11px] text-slate-400 mt-1">Queue #{hold.queuePosition}</p>
                    )}
                  </div>
                </div>
              )
