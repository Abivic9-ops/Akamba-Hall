    ...
    <div className="min-h-screen bg-[#F8F9FB] dark:bg-[#071224] dark:bg-[#071224]">
      <div className="max-w-[1200px] mx-auto p-6 space-y-5">
        <div>
          ...
          <p className="text-[15px] text-slate-500 dark:text-[#6B7A99] dark:text-[#6B7A99] mt-1">Search the library collection by title, author, subject, or ISBN.</p>
        </div>
        ...
        <SectionCard title="Search" icon={Search}>
          <div className="flex gap-3 mb-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full h-11 pl-10 pr-4 rounded-xl border border-slate-200 dark:border-white/10 dark:border-white/10 bg-white dark:bg-[#0E1F3F] dark:bg-[#0E1F3F] text-[14px] text-slate-800 dark:text-[#E2E8F0] dark:text-[#E2E8F0] placeholder:text-slate-400 dark:text-[#6B7A99] dark:text-[#6B7A99] focus:outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/10 transition-all"
              />
            </div>
          ...
          <div className="flex items-center gap-2">
            <Filter className="h-3.5 w-3.5 text-slate-400 dark:text-[#6B7A99] dark:text-[#6B7A99]" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <SectionCard title="Total Titles">
            <p className="text-[32px] font-bold text-slate-900 dark:text-[#E2E8F0] dark:text-[#E2E8F0]">{books.length}</p>
            <p className="text-[13px] text-slate-400 dark:text-[#6B7A99] dark:text-[#6B7A99] mt-1">In search results</p>
          </SectionCard>
          <SectionCard title="Available">
            <p className="text-[32px] font-bold text-[#18A957]">{books.filter((b) => b.status === 'available').length}</p>
            <p className="text-[13px] text-slate-400 dark:text-[#6B7A99] dark:text-[#6B7A99] mt-1">Ready to borrow</p>
          </SectionCard>
          <SectionCard title="On Loan">
            <p className="text-[32px] font-bold text-[#2563EB]">{books.filter((b) => b.status === 'on_loan').length}</p>
            <p className="text-[13px] text-slate-400 dark:text-[#6B7A99] dark:text-[#6B7A99] mt-1">Currently checked out</p>
          </SectionCard>
        </div>
        ...
        <SectionCard title="Results" icon={BookOpen}>
          <div className="space-y-0">
            {books.map((book) => {
              ...
              return (
                <div key={book.id} className="flex items-center gap-4 py-3 border-b border-slate-50 last:border-0 hover:bg-slate-50 dark:hover:bg-white/[0.04] dark:bg-white/[0.04] dark:hover:bg-white dark:bg-[#0E1F3F]/[0.04] dark:bg-white/[0.04] rounded-lg px-2 transition-colors">
                  <div className="h-12 w-12 rounded-lg bg-gradient-to-br from-slate-100 to-slate-50 flex items-center justify-center shrink-0 border border-slate-100 dark:border-white/[0.08] dark:border-white/[0.08]">
                    <BookOpen className="h-5 w-5 text-slate-400 dark:text-[#6B7A99] dark:text-[#6B7A99]" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[14px] font-medium text-slate-800 dark:text-[#E2E8F0] dark:text-[#E2E8F0] truncate">{book.title}</p>
                    <p className="text-[12px] text-slate-400 dark:text-[#6B7A99] dark:text-[#6B7A99] mt-0.5">{book.author} · {book.year}</p>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    <Star className="h-3 w-3 text-amber-400 fill-amber-400" />
                    <span className="text-[12px] font-medium text-slate-600 dark:text-[#94A3B8] dark:text-[#94A3B8]">{book.rating}</span>
                  </div>
                  <span className="text-[11px] text-slate-400 dark:text-[#6B7A99] dark:text-[#6B7A99] shrink-0">{book.copies} cop{book.copies !== 1 ? 'ies' : 'y'}</span>
